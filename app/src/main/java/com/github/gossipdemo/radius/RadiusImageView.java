...
public class RadiusImageView extends AppCompatImageView {
  ...
  public RadiusImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RadiusImageView, defStyle, 0);

    int index = a.getInt(R.styleable.RadiusImageView_android_scaleType, -1);
    if (index >= 0) {
    ...
    float cornerRadiusOverride =
        a.getDimensionPixelSize(R.styleable.RadiusImageView_rv_corner_radius, -1);

    mCornerRadii[Corner.TOP_LEFT] =
        a.getDimensionPixelSize(R.styleable.RadiusImageView_rv_corner_radius_top_left, -1);
    mCornerRadii[Corner.TOP_RIGHT] =
        a.getDimensionPixelSize(R.styleable.RadiusImageView_rv_corner_radius_top_right, -1);
    mCornerRadii[Corner.BOTTOM_RIGHT] =
        a.getDimensionPixelSize(R.styleable.RadiusImageView_rv_corner_radius_bottom_right, -1);
    mCornerRadii[Corner.BOTTOM_LEFT] =
        a.getDimensionPixelSize(R.styleable.RadiusImageView_rv_corner_radius_bottom_left, -1);
    ...
    mBorderWidth = a.getDimensionPixelSize(R.styleable.RadiusImageView_rv_border_width, -1);
    if (mBorderWidth < 0) {
      mBorderWidth = DEFAULT_BORDER_WIDTH;
    }

    mBorderColor = a.getColorStateList(R.styleable.RadiusImageView_rv_border_color);
    if (mBorderColor == null) {
      mBorderColor = ColorStateList.valueOf(RadiusDrawable.DEFAULT_BORDER_COLOR);
    }

    mMutateBackground = a.getBoolean(R.styleable.RadiusImageView_rv_mutate_background, false);
    mIsOval = a.getBoolean(R.styleable.RadiusImageView_rv_oval, false);

    final int tileMode = a.getInt(R.styleable.RadiusImageView_rv_tile_mode, TILE_MODE_UNDEFINED);
    ...
    final int tileModeX =
        a.getInt(R.styleable.RadiusImageView_rv_tile_mode_x, TILE_MODE_UNDEFINED);
    if (tileModeX != TILE_MODE_UNDEFINED) {
      setTileModeX(parseTileMode(tileModeX));
    }

    final int tileModeY =
        a.getInt(R.styleable.RadiusImageView_rv_tile_mode_y, TILE_MODE_UNDEFINED);
  ...
  public void setImageDrawable(Drawable drawable) {
    mResource = 0;
    mDrawable = RadiusDrawable.fromDrawable(drawable);
    updateDrawableAttrs();
  ...
  public void setImageBitmap(Bitmap bm) {
    mResource = 0;
    mDrawable = RadiusDrawable.fromBitmap(bm);
    updateDrawableAttrs();
    super.setImageDrawable(mDrawable);
  }
  ...
  private Drawable resolveResource() {
    ...
    return RadiusDrawable.fromDrawable(d);
  }
  ...
  private void updateBackgroundDrawableAttrs(boolean convert) {
    if (mMutateBackground) {
      if (convert) {
        mBackgroundDrawable = RadiusDrawable.fromDrawable(mBackgroundDrawable);
      }
  ...
  private void updateAttrs(Drawable drawable, ScaleType scaleType) {
    ...
    if (drawable instanceof RadiusDrawable) {
      ((RadiusDrawable) drawable)
      ...
      if (mCornerRadii != null) {
        ((RadiusDrawable) drawable).setCornerRadius(
            mCornerRadii[Corner.TOP_LEFT],
            mCornerRadii[Corner.TOP_RIGHT],
            mCornerRadii[Corner.BOTTOM_RIGHT],
            mCornerRadii[Corner.BOTTOM_LEFT]);
  ...
  public void setBorderColor(ColorStateList colors) {
    ...
    mBorderColor =
        (colors != null) ? colors : ColorStateList.valueOf(RadiusDrawable.DEFAULT_BORDER_COLOR);
    updateDrawableAttrs();
    updateBackgroundDrawableAttrs(false);
    if (mBorderWidth > 0) {
      invalidate();
   ...
   * width.
   *
   * @return if this {@link RadiusImageView} is set to oval.
   */
   ...
   *
   * @param oval if this {@link RadiusImageView} should be oval.
   */
  ...
  /**
   * Set whether the {@link RadiusImageView} should round the background drawable according to
